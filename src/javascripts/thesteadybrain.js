...
var elastic_color_picker = function (color_count) {
  var color_picker = d3.scale.ordinal()
                      .domain(d3.range(color_count))
                      .range(color_palete_hex(color_count));
  return color_picker;
}
...
var trace_svg = d3.select("body").append("div").attr("id", "trace")
                .append("svg");
visualizeTrace(null, null);
...
function module_color2 (data, color_key) {
  var color_count = 0;
  var color_group = null;

  switch(color_key) {
    case "class":
      color_count = module_count;
      color_group = data.colorGroup2;
      break;
    case "cluster":
      color_count = cluster_count;
      color_group = data.colorGroup;
      break;
    case "community":
      color_count = community_count;
      color_group = data.community;
      break;
  }
  ...
  var color_picker = elastic_color_picker(color_count);
  return color_picker(color_group);
}

function module_color (data) {
  return module_color2(data, color_key);
}

var is_stop_class = function(class_name) {
  if(class_name === null
      || class_name.length === 0
      || class_name.toLowerCase().indexOf("test") != -1) {
    return true;
  }
  return false;
}
...
var init = function(error, graph) {
  ...
  var useMethodNameForColor = (all_classes.length == 1);

  if(useMethodNameForColor) {
    ddg_nodes.forEach(function(element) {
      var class_name = element.className + "." + element.methodName;
      // if(is_stop_class(class_name)) return;
  ...
  ddg_nodes.forEach(function(element) {
    var class_name;
    if(useMethodNameForColor) {
      class_name = element.className + "." + element.methodName;
    } else {
  ...
  module_count = all_classes.length;
  
  ddg_nodes.forEach(function(element) {
    if(element.colorGroup > cluster_count) {
      cluster_count = element.colorGroup;
    }
  });
  cluster_count += 1;

  ddg_nodes.forEach(function(element) {
    if(element.community > community_count) {
      community_count = parseInt(element.community);
    }
  });
  community_count += 1;
...
initate(document.getElementById("subject-selector").value);
...
var blink3 = function() {
  ...
  d3.json("traces/" + "nanoxml" + "/" + trace_name + ".json", function(error, json) {
    ...
    trace = json;
    visualizeTrace(trace, ddg_nodes);
    playstate.innerHTML = "Playing";
    var slider = document.getElementsByName("slider")[0];
...
var get_instruction_info = function(insn, sep) {
  ...
  str_temp += sep + "Source Code Line: " + insn.cluster;
  str_temp += sep + "Community: " + insn.community;
  return str_temp;
}

var change_color_key = function(value) {
  color_key = value;
}
